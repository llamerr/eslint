...
module.exports = {
    ...
    create(context) {
        ...
        function checkVariableDeclaration(node) {
            ...
            if (nodeIndex > 0) {
                const previousNode = parent.body && parent.body[nodeIndex - 1];
            ...
            if (!hasOnlyOneStatement(type, declarations)) {
                if (options[type].initialized === MODE_ALWAYS && options[type].uninitialized === MODE_ALWAYS) {
                    ...
                    if (options[type].initialized === MODE_ALWAYS && options[type].uninitialized !== MODE_CONSECUTIVE) {
                        context.report({
                            node,
                            message: "Combine this with the previous '{{type}}' statement with initialized variables.",
                            data: {
                    ...
                    if (options[type].uninitialized === MODE_ALWAYS && options[type].initialized !== MODE_CONSECUTIVE) {
